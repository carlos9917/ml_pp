...
class RoadTempDataset(Dataset):
    """Custom Dataset for road temperature profiles"""
...
class TempProfilePredictor(pl.LightningModule):
    """Neural network for temperature profile prediction"""
...
def create_training_pipeline(train_path, test_path, batch_size=32, num_epochs=100):
    """Sets up and executes the training pipeline"""

    # Initialize preprocessor and load data
    preprocessor = DataPreprocessor(train_path, test_path)
    train_sequences, train_targets, test_sequences, test_targets = preprocessor.load_and_preprocess()

    # Create datasets
    train_dataset = RoadTempDataset(train_sequences, train_targets)
    val_dataset = RoadTempDataset(test_sequences, test_targets)
